    ...
    <div >
      ...
      <div className="pnum" ref={peapleRef}>
        ...
        <div className="num_action_wrapper">
          <button className="num_leftbtn btn" name="m" onClick={onNumClick}>-</button>
          <div className="num">{peapleCount}</div>
          <button className="num_rightbtn btn" name="p" onClick={onNumClick}>+</button>

        </div>

      </div>
  
      <div className="clock6" ><input className="clock_check" type="checkbox" />

      <span>6시부터 입장({remain}/100석)</span></div>


        <div className="input_wrapper">
          <label htmlFor="numberInput">
            <div className="labeldiv">대표자의 학번을 입력해주세요</div>
            <input id="numberInput" className="input_box" name="number" type="number" placeholder="학번 입력" ref={numRef} onChange={(e)=>setNumber(e.target.value)}/>
          </label>
          <label htmlFor="nameInput">
          <div className="labeldiv">대표자의 이름을 입력해주세요</div>
            <input id="nameInput" className="input_box" name="name" type="text" placeholder="이름 입력" ref={nameRef} onChange={(e)=>setName(e.target.value)}/>
          </label>
          <label htmlFor="phoneInput">
          <div className="labeldiv">대표자의 전화번호를 입력해주세요</div>
            <input id="phoneInput" className="input_box" name="phone" type="number" placeholder="- 빼고 숫자만 입력" ref={phoneRef} onChange={(e)=>setPhone(e.target.value)}/>
          </label>
        </div>
        ...
        <button className="checkbtn" onClick={onButtonClick}>확인</button>
      </div>
      )}
    </div>
  );
